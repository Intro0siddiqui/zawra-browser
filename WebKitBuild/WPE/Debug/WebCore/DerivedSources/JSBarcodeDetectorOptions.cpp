...
template<> BarcodeDetectorOptions convertDictionary<BarcodeDetectorOptions>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    if (!formatsValue.isUndefined()) {
        result.formats = convert<IDLSequence<IDLEnumeration<BarcodeFormat>>>(lexicalGlobalObject, formatsValue);
        RETURN_IF_EXCEPTION(throwScope, { });
    }
    return result;
}
