...
template<> GPUComputePassDescriptor convertDictionary<GPUComputePassDescriptor>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    GPUComputePassDescriptor result;
    JSValue labelValue;
    ...
    if (!timestampWritesValue.isUndefined()) {
        result.timestampWrites = convert<IDLSequence<IDLDictionary<GPUComputePassTimestampWrite>>>(lexicalGlobalObject, timestampWritesValue);
        RETURN_IF_EXCEPTION(throwScope, { });
    } else
        result.timestampWrites = Converter<IDLSequence<IDLDictionary<GPUComputePassTimestampWrite>>>::ReturnType{ };
    return result;
}
