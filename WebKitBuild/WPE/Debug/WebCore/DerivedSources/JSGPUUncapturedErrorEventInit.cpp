...
template<> GPUUncapturedErrorEventInit convertDictionary<GPUUncapturedErrorEventInit>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    if (!errorValue.isUndefined()) {
        result.error = convert<IDLUnion<IDLInterface<GPUOutOfMemoryError>, IDLInterface<GPUValidationError>, IDLInterface<GPUInternalError>>>(lexicalGlobalObject, errorValue);
        RETURN_IF_EXCEPTION(throwScope, { });
    } else {
        throwRequiredMemberTypeError(lexicalGlobalObject, throwScope, "error", "GPUUncapturedErrorEventInit", "(GPUOutOfMemoryError or GPUValidationError or GPUInternalError)");
        return { };
    }
    return result;
}
